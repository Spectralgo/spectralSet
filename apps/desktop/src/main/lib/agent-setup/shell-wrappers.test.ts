			...
			const output = execFileSync(
				...
				["-ic", `source "${wrapperPath}"; node`],
				{
					...
					env: {
						...
						SPECTRALSET_ORIG_ZDOTDIR: homeDir,
...
`,
	...
	describe("SPECTRALSET_* env var protection from user RC overrides", () => {
		it("bash wrapper restores SPECTRALSET_WORKSPACE_NAME after user .bashrc overrides it", () => {
			const integrationRoot = path.join(TEST_ROOT, "bash-env-protect");
			const homeDir = path.join(integrationRoot, "home");
			mkdirSync(homeDir, { recursive: true });

			// User .bashrc overrides SPECTRALSET_WORKSPACE_NAME with corrupted value
			writeFileSync(
				path.join(homeDir, ".bashrc"),
				`export SPECTRALSET_WORKSPACE_NAME="user@host:~/path/to/worktree"\n`,
			);
			...
			const args = [
				...
				path.join(TEST_BASH_DIR, "rcfile"),
				"-ic",
				'echo "$SPECTRALSET_WORKSPACE_NAME"',
			];
			const output = execFileSync("bash", args, {
				encoding: "utf-8",
				env: {
					HOME: homeDir,
					PATH: "/usr/bin:/bin",
					SPECTRALSET_WORKSPACE_NAME: "my-clean-workspace",
				},
			}).trim();
		...
		it("bash wrapper restores SPECTRALSET_WORKSPACE_NAME after user .bash_profile overrides it", () => {
			const integrationRoot = path.join(TEST_ROOT, "bash-profile-env-protect");
			const homeDir = path.join(integrationRoot, "home");
			mkdirSync(homeDir, { recursive: true });

			// User .bash_profile overrides SPECTRALSET_WORKSPACE_NAME
			writeFileSync(
				path.join(homeDir, ".bash_profile"),
				`export SPECTRALSET_WORKSPACE_NAME="$(whoami)@$(hostname):$(pwd)"\n`,
			);
			...
			const output = execFileSync("bash", args, {
				...
				env: {
					HOME: homeDir,
					PATH: "/usr/bin:/bin",
					SPECTRALSET_WORKSPACE_NAME: "correct-name",
				},
			}).trim();
		...
		it("bash wrapper restores multiple SPECTRALSET_* vars after user RC overrides them", () => {
			...
			writeFileSync(
				path.join(homeDir, ".bashrc"),
				`export SPECTRALSET_WORKSPACE_NAME="corrupted"
export SPECTRALSET_WORKSPACE_PATH="/wrong/path"
`,
			);
			...
			const args = [
				...
				path.join(TEST_BASH_DIR, "rcfile"),
				"-ic",
				'echo "$SPECTRALSET_WORKSPACE_NAME|$SPECTRALSET_WORKSPACE_PATH"',
			];
			const output = execFileSync("bash", args, {
				encoding: "utf-8",
				env: {
					...
					PATH: "/usr/bin:/bin",
					SPECTRALSET_WORKSPACE_NAME: "correct-name",
					SPECTRALSET_WORKSPACE_PATH: "/correct/path",
				},
			}).trim();
		...
		it("zsh wrapper restores SPECTRALSET_WORKSPACE_NAME after user .zshrc overrides it", () => {
			if (!isZshAvailable()) return;
			...
			// User .zshrc overrides SPECTRALSET_WORKSPACE_NAME with corrupted value
			writeFileSync(
				path.join(homeDir, ".zshrc"),
				`export SPECTRALSET_WORKSPACE_NAME="user@host:~/path/to/worktree"\n`,
			);
			...
			const output = execFileSync(
				"zsh",
				["-lic", 'echo "$SPECTRALSET_WORKSPACE_NAME"'],
				{
					encoding: "utf-8",
					env: {
						HOME: homeDir,
		...
		it("zsh wrapper restores SPECTRALSET_WORKSPACE_NAME after user .zlogin overrides it", () => {
			if (!isZshAvailable()) return;
			...
			writeFileSync(
				path.join(homeDir, ".zlogin"),
				`export SPECTRALSET_WORKSPACE_NAME="overridden-by-zlogin"\n`,
			);
