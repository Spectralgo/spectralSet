...
SpectralSet includes a built-in chat panel for AI interactions without leaving the app.
...
- **Model Override** - Optional model override for SpectralSet Chat
...
- **On** - Terminal agents run their command right away, and SpectralSet Chat sends the task prompt immediately
- **Off** - Terminal agents stage the command without pressing Enter, and SpectralSet Chat opens as a draft instead
