...
const TEST_SPECTRALSET_HOME = join(TEST_DIR, "superset-home");
...
const ORIGINAL_SPECTRALSET_HOME_DIR = process.env.SPECTRALSET_HOME_DIR;
...
describe("runTeardown", () => {
	beforeEach(() => {
		process.env.SPECTRALSET_HOME_DIR = TEST_SPECTRALSET_HOME;
		// Create test directories
		mkdirSync(join(MAIN_REPO, ".spectralset"), { recursive: true });
	...
	afterEach(() => {
		...
		if (ORIGINAL_SPECTRALSET_HOME_DIR === undefined) {
			delete process.env.SPECTRALSET_HOME_DIR;
		} else {
			process.env.SPECTRALSET_HOME_DIR = ORIGINAL_SPECTRALSET_HOME_DIR;
		}
	...
	test("returns success when config has no teardown commands", async () => {
		writeFileSync(
			join(MAIN_REPO, ".spectralset", "config.json"),
	...
	test("uses worktreePath config when present", async () => {
		...
		mkdirSync(join(WORKTREE, ".spectralset"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".spectralset", "config.json"),
	...
	test("sets environment variables for teardown scripts", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				teardown: [
					`echo "$SPECTRALSET_WORKSPACE_NAME|$SPECTRALSET_ROOT_PATH" > "${envFile}"`,
				],
			}),
		);
	...
	test("uses managed wrapper path for teardown managed binaries even with PATH rewrites", async () => {
		...
		const wrapperBinDir = join(TEST_SPECTRALSET_HOME, "bin");
