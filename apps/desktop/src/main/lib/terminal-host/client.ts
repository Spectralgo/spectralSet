...
const DEBUG_CLIENT = process.env.SPECTRALSET_TERMINAL_DEBUG === "1";
...
const SOCKET_PATH = join(SPECTRALSET_HOME_DIR, "terminal-host.sock");
const TOKEN_PATH = join(SPECTRALSET_HOME_DIR, "terminal-host.token");
const PID_PATH = join(SPECTRALSET_HOME_DIR, "terminal-host.pid");
const SPAWN_LOCK_PATH = join(SPECTRALSET_HOME_DIR, "terminal-host.spawn.lock");
const SCRIPT_MTIME_PATH = join(SPECTRALSET_HOME_DIR, "terminal-host.mtime");
...
export class TerminalHostClient extends EventEmitter {
	...
	private acquireSpawnLock(): boolean {
		try {
			...
			try {
				chmodSync(SPECTRALSET_HOME_DIR, 0o700);
			} catch {
				// Best-effort.
			}
	...
	private async spawnDaemon(): Promise<void> {
		...
		try {
			...
			const logPath = join(SPECTRALSET_HOME_DIR, "daemon.log");
			let logFd: number;
			try {
