...
SpectralSet does not assign per-workspace port ranges. It discovers listening ports from processes running in each workspace and lets you manage them from the UI.
...
Create `.spectralset/ports.json` in your repository:
...
**Tips:**
- Commit `.spectralset/ports.json` to share port labels with your team
- **Pro tip:** If you want deterministic per-workspace port ranges, implement it in setup/teardown scripts by reserving a range in a shared file (for example `~/.spectralset/port-allocations.json`) during setup and releasing it during teardown. See this repo's examples: [`.spectralset/setup.sh`](https://github.com/Spectralgo/spectralSet/blob/main/.spectralset/setup.sh) and [`.spectralset/teardown.sh`](https://github.com/Spectralgo/spectralSet/blob/main/.spectralset/teardown.sh).
