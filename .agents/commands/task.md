---
description: Create a task and assign it to me via SpectralSet MCP
allowed-tools: mcp__spectralset__create_task, mcp__spectralset__list_members, Bash(git config user.email)
---

Create a new task in SpectralSet and assign it to me.
...
4. Resolve the current user's member ID by calling `mcp__spectralset__list_members` and matching against the git user (run `git config user.email` to get the current user's email)
5. Create the task using `mcp__spectralset__create_task` with:
