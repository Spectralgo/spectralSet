...
Research and refresh SpectralSet marketing comparison content using current official sources, not stale model memory.
...
- Use fresh official sources for every unstable claim:
  - SpectralSet: `spectralset.dev`, `docs.spectralset.dev`, SpectralSet changelog, and relevant repo/docs files
...
- SpectralSet being framed too narrowly when the current product includes more surface area
...
- make the body explicitly enumerate:
  ...
  - important SpectralSet positioning changes added to the copy
...
- `marketing: refresh compare pages with current SpectralSet chat/browser/MCP positioning`
