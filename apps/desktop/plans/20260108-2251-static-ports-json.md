...
- Observation: File watching needed additional logic to handle both file and directory watching
  Evidence: When ports.json doesn't exist, we watch the .spectralset directory for file creation; when it exists, we watch the file directly for changes

## Decision Log

- Decision: Workspace-scoped ports.json reading
  Rationale: User requested this approach. Each workspace's worktree has its own `.spectralset/ports.json`, allowing different branches to have different port configurations.
...
- Constants defined in `apps/desktop/src/shared/constants.ts`: `PROJECT_SPECTRALSET_DIR_NAME = ".spectralset"`, `CONFIG_FILE_NAME = "config.json"`
...
8. **Tips section**:
   ...
   - Share port documentation with your team by committing `.spectralset/ports.json`
...
For manual validation:
    ...
    # In the workspace's worktree, create .spectralset/ports.json:
...
4. **Manual test - no config**: With no `.spectralset/ports.json`, dynamic port discovery works as before
