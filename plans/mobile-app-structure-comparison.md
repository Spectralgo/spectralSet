...
This document compares the organizational patterns between the Cadra mobile app (reference monorepo) and the SpectralSet mobile app to understand best practices for Expo Router + screens/ colocation.
...
### SpectralSet (Current Implementation)
```
apps/mobile/
...
| Aspect | Cadra Pattern | SpectralSet Pattern |
|--------|---------------|------------------|
...
⚠️ **Key Difference:** Cadra keeps root layout in `app/`, SpectralSet moves it to `screens/`
...
⚠️ **Key Difference:** Cadra keeps redirect-only routes in `app/`, SpectralSet moves them to `screens/`
...
**SpectralSet:**
```tsx
// If we had tabs, following SpectralSet pattern:
...
⚠️ **Key Difference:** Cadra keeps `_layout.tsx` in `app/` for navigation config, SpectralSet would move to `screens/`

---

### Pattern 5: Component Colocation

**Both Cadra & SpectralSet use the SAME pattern:**
...
To migrate SpectralSet to follow Cadra's pattern:
...
- **SpectralSet**: `screens/` owns everything, `app/` is just a thin routing proxy
