...
report_violation \
	"[git-refs] '.startsWith(\"origin/\")' is forbidden — a local branch can be named 'origin/foo' and would be misclassified. Use ResolvedRef from @spectralset/host-service/git." \
	"\\.startsWith\\(\\s*['\"]origin/" \
	--type ts \
	--glob '!**/*.test.ts' \
