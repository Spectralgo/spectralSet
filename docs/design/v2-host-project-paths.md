...
2. Or auto-clones to a hardcoded path: `~/.spectralset/repos/{projectId}`
...
| `packages/host-service/src/trpc/router/workspace/workspace.ts` | Workspace creation — auto-clones to `~/.spectralset/repos/{projectId}` if missing |
...
workspace.create(projectId, name, branch)
  → local project exists? → YES → create worktree from repoPath
                           → NO  → auto-clone to ~/.spectralset/repos/{projectId}
