...
const repo = { owner: "Spectralgo", name: "spectralSet" };
	...
	describe("non-GitHub URLs (plain text fallback)", () => {
		test("GitHub repo URL (no entity path)", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
		...
		test("GitHub compare URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/compare/main...feature",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
		...
		test("SSH-style URL", () => {
			const result = normalizeGitHubQuery(
				"git@github.com:Spectralgo/spectralSet.git",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
...
describe("normalizeGitHubQuery — PR URLs", () => {
	describe("same repo", () => {
		test("basic URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3130",
				repo,
				"pull",
			);
			expect(result.query).toBe("3130");
		...
		test("/files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/files",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/changes tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/changes",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/commits tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/commits",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/checks tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/checks",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("trailing slash", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354?diff=unified",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params on tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/files?diff=split&w=1",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354#discussion_r123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment on files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/pull/3354/files#diff-abc123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("www prefix", () => {
			const result = normalizeGitHubQuery(
				"https://www.github.com/Spectralgo/spectralSet/pull/3354",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("http (not https)", () => {
			const result = normalizeGitHubQuery(
				"http://github.com/Spectralgo/spectralSet/pull/3354",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("case-insensitive owner/repo", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/SPECTRALGO/SPECTRALSET/pull/100",
				repo,
				"pull",
			);
	...
	describe("cross-repo mismatch", () => {
		test("different owner", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/other-org/spectralSet/pull/100",
				repo,
				"pull",
			);
		...
		test("different repo name", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/other-repo/pull/100",
				repo,
				"pull",
			);
			expect(result.query).toBe("");
...
describe("normalizeGitHubQuery — issue URLs", () => {
	describe("same repo", () => {
		test("basic URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/issues/100",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/issues/100?q=1",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("hash fragment (comment anchor)", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/spectralSet/issues/100#issuecomment-12345",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("www prefix", () => {
			const result = normalizeGitHubQuery(
				"https://www.github.com/Spectralgo/spectralSet/issues/200",
				repo,
				"issue",
			);
		...
		test("http (not https)", () => {
			const result = normalizeGitHubQuery(
				"http://github.com/Spectralgo/spectralSet/issues/200",
				repo,
				"issue",
			);
			expect(result.query).toBe("200");
		...
		test("case-insensitive owner/repo", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/SPECTRALGO/SPECTRALSET/issues/55",
				repo,
				"issue",
			);
	...
	describe("cross-repo mismatch", () => {
		test("different owner", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/other-org/spectralSet/issues/100",
				repo,
				"issue",
			);
			expect(result.query).toBe("");
		...
		test("different repo", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/Spectralgo/other-repo/issues/100",
				repo,
				"issue",
			);
			expect(result.query).toBe("");
