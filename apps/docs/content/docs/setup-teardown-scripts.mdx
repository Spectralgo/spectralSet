...
Create `.spectralset/config.json` in your project:

```json
{
  "setup": ["bun install", "cp \"$SPECTRALSET_ROOT_PATH/.env\" .env"],
  "teardown": ["docker-compose down"],
  "run": ["./.spectralset/run.sh"]
}
```
...
|----------|-------------|
| `SPECTRALSET_ROOT_PATH` | Path to root repository |
| `SPECTRALSET_WORKSPACE_NAME` | Current workspace name |
| `SPECTRALSET_WORKSPACE_PATH` | Path to the workspace worktree |
...
```
~/.spectralset/projects/<project-id>/config.json
```

Where `<project-id>` is your project's unique ID in SpectralSet (visible in the app's project settings).

### Priority Order

Config is resolved in this order (first found wins, for setup, teardown, and run):
1. `~/.spectralset/projects/<project-id>/config.json` — user override
2. `<worktree>/.spectralset/config.json` — worktree-specific
3. `<repo>/.spectralset/config.json` — project default
...
```json
{
  "setup": ["~/.spectralset/projects/<project-id>/setup.sh"],
  "teardown": ["~/.spectralset/projects/<project-id>/teardown.sh"]
}
```
...
Extend or override your team's committed scripts without modifying the repo. Create `.spectralset/config.local.json` alongside `config.json` — it's gitignored automatically.
...
```json
{
  "setup": {
    ...
    "after": ["./.spectralset/my-post-setup.sh"]
  },
  "teardown": {
    "after": ["./.spectralset/my-cleanup.sh"]
  }
}
```
...
```json
{
  "setup": ["./.spectralset/my-custom-setup.sh"]
}
```
...
- Commit `.spectralset/` to share with team
- Use shell scripts for complex logic: `"setup": ["./.spectralset/setup.sh"]`
- Use `config.local.json` to add personal steps without touching the team's config
- Use user overrides (`~/.spectralset/projects/`) to replace scripts entirely per-project
