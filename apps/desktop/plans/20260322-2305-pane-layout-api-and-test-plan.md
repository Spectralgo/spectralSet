...
Define a shippable SpectralSet pane-layout package before we continue implementation.
...
- Renderer layer must be design-system-native, using Tailwind + `@spectralset/ui`
...
- `react`
  ...
  - group/tab chrome built from `@spectralset/ui`
...
This package is internal to SpectralSet. It does not need an external-consumer-neutral API.
...
New for SpectralSet because cross-root drag/drop is a core requirement.
...
1. Lock the core API in `@spectralset/pane-layout`
2. Write reducer and tree utility tests first
3. Implement root-aware reducer
4. Implement minimal React renderer with `@spectralset/ui` primitives
...
- a SpectralSet-specific React renderer
