---
title: MCP Server
description: Connect AI agents directly to SpectralSet via MCP
---

## Overview

SpectralSet provides an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) server that lets AI agents interact with your tasks, workspaces, and devices programmatically.
...
<Tab value="Amp CLI">
```bash title="terminal"
amp mcp add --workspace spectralset https://api.spectralset.dev/api/agent/mcp
```
</Tab>
<Tab value="Claude Code">
```bash title="terminal"
claude mcp add spectralset --transport http https://api.spectralset.dev/api/agent/mcp
```
</Tab>
<Tab value="Codex">
```bash title="terminal"
codex mcp add spectralset --url https://api.spectralset.dev/api/agent/mcp
```
</Tab>
<Tab value="Gemini CLI">
```bash title="terminal"
gemini mcp add --transport http spectralset https://api.spectralset.dev/api/agent/mcp
```
</Tab>
<Tab value="Open Code">
...
[2m│[0m  [36mspectralset[0m
[2m│[0m
[2m◇[0m  Select MCP server type
[2m│[0m  [36mRemote[0m
[2m│[0m
[2m◇[0m  Enter MCP server URL
[2m│[0m  [36mhttps://api.spectralset.dev/api/agent/mcp[0m
[2m│[0m
...
```json title=".amp/settings.json"
{
  "amp.mcpServers": {
    "spectralset": {
      "url": "https://api.spectralset.dev/api/agent/mcp"
    }
  }
}
```
...
```json title="settings.json"
{
  "mcpServers": {
    "spectralset": {
      "httpUrl": "https://api.spectralset.dev/api/agent/mcp"
    }
  }
}
```
...
1. Open the SpectralSet desktop app
...
| Tool | Description |
|------|-------------|
| `start_agent_session` | Start an autonomous AI agent session for a task in an existing workspace. Requires `taskId`. Supports Amp, Claude, Codex, Gemini, OpenCode, Pi, Copilot, Cursor Agent, and SpectralSet Chat (defaults to Claude). When `paneId` is provided, adds a new pane to the tab containing that pane instead of initializing a new tab. |
