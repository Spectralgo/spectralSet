...
`@spectralset/panes` store, call `workspaceTrpc.terminal.ensureSession` to
attach PTY.
...
│         .writeFile → <worktree>/.spectralset/attachments/…     │
...
separate `@spectralset/panes` store, so launches dispatched through V1
...
- `routes/.../v2-workspace/$workspaceId/hooks/useConsumePendingLaunch/*` — mount-effect hook that live-queries the pending row, opens a pane via V2 `@spectralset/panes` store, writes the command via `workspaceTrpc`, clears the field.
...
- [ ] **Prompt + attachment**. Drop a small text file. Submit. File lands at `<worktree>/.spectralset/attachments/<filename>`; prompt includes `- .spectralset/attachments/<filename>`.
...
1. **Deep solve for binary transport.** Current fix for the
   ...
   Library-level change to `@spectralset/ui/ai-elements/prompt-input`
