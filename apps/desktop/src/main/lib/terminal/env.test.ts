...
describe("env", () => {
	...
	describe("buildSafeEnv", () => {
		...
		describe("includes SPECTRALSET_* prefix vars", () => {
			it("should include SPECTRALSET_* vars (our metadata)", () => {
				const env = {
					SPECTRALSET_TERMINAL_ID: "pane-1",
					SPECTRALSET_TAB_ID: "tab-1",
					SPECTRALSET_WORKSPACE_ID: "ws-1",
					PATH: "/usr/bin",
				};
				const result = buildSafeEnv(env);
				expect(result.SPECTRALSET_TERMINAL_ID).toBe("pane-1");
				expect(result.SPECTRALSET_TAB_ID).toBe("tab-1");
				expect(result.SPECTRALSET_WORKSPACE_ID).toBe("ws-1");
			});
		});
		...
		describe("Windows platform case-insensitivity", () => {
			...
			it("should include Spectralset_* prefix vars case-insensitively on Windows", () => {
				const env = {
					Spectralset_Terminal_Id: "pane-1",
					SPECTRALSET_TAB_ID: "tab-1",
					PATH: "/usr/bin",
	...
	describe("buildTerminalEnv", () => {
		...
		describe("terminal metadata", () => {
			...
			it("should set Spectralset-specific env vars", () => {
				const result = buildTerminalEnv(baseParams);
			...
			it("should handle optional workspace params", () => {
				...
				expect(result.SPECTRALSET_WORKSPACE_NAME).toBe("my-workspace");
				expect(result.SPECTRALSET_WORKSPACE_PATH).toBe("/path/to/workspace");
				expect(result.SPECTRALSET_ROOT_PATH).toBe("/root/path");
			});

			it("should default optional params to empty string", () => {
				const result = buildTerminalEnv(baseParams);

				expect(result.SPECTRALSET_WORKSPACE_NAME).toBe("");
				expect(result.SPECTRALSET_WORKSPACE_PATH).toBe("");
				expect(result.SPECTRALSET_ROOT_PATH).toBe("");
			});
			...
			it("should include SPECTRALSET_AGENT_HOOK_PORT", () => {
				const result = buildTerminalEnv(baseParams);
				expect(result.SPECTRALSET_AGENT_HOOK_PORT).toBeDefined();
				expect(typeof result.SPECTRALSET_AGENT_HOOK_PORT).toBe("string");
			});

			it("should preserve SPECTRALSET_HOME_DIR for app-launched hooks", () => {
				process.env.SPECTRALSET_HOME_DIR = "/tmp/superset-home";
				const result = buildTerminalEnv(baseParams);
				expect(result.SPECTRALSET_HOME_DIR).toBe("/tmp/superset-home");
			});
		});

		it("should include SPECTRALSET_ENV for dev/prod separation", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.SPECTRALSET_ENV).toBeDefined();
			expect(["development", "production"]).toContain(result.SPECTRALSET_ENV);
		});

		it("should include SPECTRALSET_AGENT_HOOK_VERSION for protocol versioning", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.SPECTRALSET_AGENT_HOOK_VERSION).toBeDefined();
