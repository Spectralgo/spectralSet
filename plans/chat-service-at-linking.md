...
packages/chat-service/
  ...
  tsconfig.json                                    @spectralset/db, @spectralset/durable-session
...
- Import `searchFiles`, `AgentManager` from `@spectralset/chat-service`
- Import Zod schemas from `@spectralset/chat-service/router`
...
```tsx
import { chatService } from "@spectralset/chat-service/client";
