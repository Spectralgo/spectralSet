...
- **`@spectralset/panes` stays generic** — `TData` parameterized by consumers
...
1. Rename `packages/pane-layout/` → `packages/panes/` and `@spectralset/panes` → `@spectralset/panes` in `package.json`
2. Delete all existing source files in `src/` (types, store, react components, tests)
3. Update the import in `apps/desktop/package.json` from `@spectralset/panes` to `@spectralset/panes`
...
```tsx
import type { PaneRegistry } from "@spectralset/panes";
...
```tsx
import { createWorkspaceStore, createTab, createPane } from "@spectralset/panes";
...
```tsx
import { Workspace } from "@spectralset/panes";
